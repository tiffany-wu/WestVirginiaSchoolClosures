...
local({
  ...
  if (dir.exists(www_dir)) {
    ...
    warning("www/ folder not found near app.R â€?images and the map iframe may 404.")
  }
})
...
# =========================
processed_data_path <- file.path("processed_data", "app_data.rds")
if (!file.exists(processed_data_path)) {
  stop(
    "Processed data file not found at ", processed_data_path,
    ". Run source('prepare_data.R') before launching the app.",
    call. = FALSE
  )
}

app_data <- readRDS(processed_data_path)
list2env(app_data, envir = environment())
rm(app_data)
